...
def create_loaders(
    ...
    train_size, test_size = int(len(dataset) * train_fraction), int(
        len(dataset) * test_fraction
    )
    val_size = len(dataset) - train_size - test_size
    train_ds, val_ds, test_ds = random_split(dataset, (train_size, val_size, test_size))
    return (
        DataLoader(
            train_ds, shuffle=True, batch_size=batch_size, collate_fn=collate_batch
        ),
        DataLoader(val_ds, batch_size=batch_size, collate_fn=collate_batch),
        DataLoader(test_ds, batch_size=batch_size, collate_fn=collate_batch),
    )
